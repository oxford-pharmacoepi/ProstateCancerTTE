...
codelist$`[OPTIMA PCa RQ4] PSA` <- cdm$concept |> dplyr::filter(concept_id %in% codelist$`[OPTIMA PCa RQ4] PSA`) |> dplyr::filter(!grepl("free", tolower(.data$concept_name))) |> dplyr::pull(concept_id)
...
cdm$psa_values <- CohortConstructor::measurementCohort(cdm,
  ...
  dplyr::left_join(cdm$measurement |>
  ...
  CohortConstructor::requireConceptIntersect(conceptSet = list("tr" = c(codelist$`[OPTIMA RQ4] EBRT`, codelist$`[OPTIMA RQ4] Radical prostatectomy`) ),
                                             window = c(0, 180),
...
cdm$stage <- CohortConstructor::conceptCohort(cdm, conceptSet = list("c_t1_t2" = codelist$`[OPTIMA PCa RQ4] cT1-T2`), name = "stage") |>
