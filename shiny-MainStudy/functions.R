...
simpleTable <- function(result,
  ...
  result <- result |>
    ...
    visOmopResults::visOmopTable(estimateName = formatEstimates,  
                             header = header,
                             groupColumn = group, 
                             hide = hide, 
                             rename = rename,
...
tidyDT <- function(x,
  ...
  if (pivotVariables) {
    varCols <- unique(x$variable_name)
    cols <- colnames(x)
    x <- x |>
      dplyr::group_by(dplyr::across(dplyr::all_of(cols[!cols %in% c("variable_name", "variable_level")]))) |>
      dplyr::mutate(row = dplyr::row_number()) |>
      dplyr::ungroup() |>
      tidyr::pivot_wider(
        id_cols    = c(cols[!cols %in% c("variable_name", "variable_level")],"row"),
    ...
    varCols <- c("variable_name", "variable_level")
  }
...
timeLimitFromInput <- function(val){
  if (val == "2 y") {
    return(731L)
  }else if (val == "5 y") {
    return(1827L)
